...
export default function ComponentDeliveryNote() {
    ...
    const fetchComponent = async () => {
      try {
        ...
        if (compSnap.exists()) {
          ...
          setComponent({ id: compSnap.id, ...data });
        ...
        alert('Error al cargar componente: ' + err.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const subtotal = component ? component.precio * unidades : 0;
  ...
  if (!component) {
    return (
      <div className="text-center py-20">
        <p className="text-gray-500">No se encontró el componente.</p>
        <Link to="/admin/components" className="text-purple-600 hover:underline mt-4 inline-block">Volver al Dashboard</Link>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Genera la nota para <strong>{component.nombre}</strong></p>
          </div>
        </div>
        <button
          onClick={handleDownloadPDF}
          className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg font-medium transition-colors flex items-center gap-2 shadow-md shadow-purple-500/20"
        >
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <select
                ...
                className="px-2 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 outline-none bg-white"
              >
        ...
        <div className="space-y-3">
              ...
              <div key={index} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <select
                    ...
                    className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 outline-none font-medium"
                  >
                  ...
                  <select
                    ...
                    className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-purple-500 outline-none text-gray-700 font-medium"
                ...
                <div className="flex items-center gap-2 flex-1">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full pl-8 pr-3 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 outline-none font-semibold"
                    />
                  </div>
                  {esEnBs && tasaNum > 0 && (
          ...
          <button
            onClick={addPago}
            className="flex items-center gap-2 text-sm text-purple-600 hover:text-purple-700 font-medium transition-colors mt-2"
          >
        ...
        <div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
          <div className="flex items-center justify-between mb-1">
            <label className="block text-sm font-medium text-purple-800">Tasa de cambio (Bs por $)</label>
            {tasaCambio > 0 && (
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-purple-700 font-medium">1 $ =</span>
            <input
              type="number" min="0" step="0.01"
              value={tasa}
              onChange={(e) => setTasa(e.target.value)}
              placeholder="Ej: 95.50"
              className="w-40 px-3 py-2 border border-purple-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-400 outline-none bg-white"
            />
            <span className="text-sm text-purple-700 font-medium">Bs</span>
            {tasaCambio > 0 && Number(tasa) !== tasaCambio && (
              <button
                type="button"
                onClick={() => setTasa(String(tasaCambio))}
                className="text-xs text-purple-800 underline hover:text-purple-900 font-medium ml-2"
              >
                Usar tasa de Finanzas
              </button>
            )}
          </div>
          <p className="text-xs text-purple-600 mt-1">Se usa para calcular el equivalente en USD de los pagos en Bs.</p>
        </div>
        ...
        <div className="mt-4">
          ...
          <textarea
            ...
            rows="5"
            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 outline-none resize-y"
          />
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div ref={noteRef} style={{ padding: '40px', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", color: '#222', background: '#fff', maxWidth: '720px', margin: '0 auto' }}>
          ...
          {/* Title */}
          <h1 style={{ textAlign: 'center', fontSize: '20px', fontWeight: '700', color: '#1a1a1a', borderBottom: '3px solid #a855f7', paddingBottom: '8px', marginBottom: '20px', letterSpacing: '2px' }}>
            NOTA DE ENTREGA - COMPONENTE
          </h1>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', fontSize: '13px' }}>
            ...
            <div>
              <span style={{ color: '#a855f7', fontWeight: '600' }}>Nro: </span>
              <span style={{ background: '#faf5ff', padding: '2px 10px', borderRadius: '4px', fontWeight: '700', border: '1px solid #e9d5ff' }}>{noteNumber}</span>
            </div>
          </div>
            ...
            return (
              <>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', marginBottom: '6px' }}>
                  <thead>
                    <tr style={{ background: '#faf5ff', borderBottom: '2px solid #a855f7' }}>
                      <th style={{ padding: '8px 10px', textAlign: 'left', color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Cantidad</th>
                      <th style={{ padding: '8px 10px', textAlign: 'left', color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Concepto/Referencia</th>
                      <th style={{ padding: '8px 10px', textAlign: 'right', color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Precio</th>
                      <th style={{ padding: '8px 10px', textAlign: 'right', color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Sub-Total</th>
                    </tr>
                  </thead>
                  <tbody>
                ...
                <div style={{ background: '#fafafa', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '12px 14px', marginBottom: '16px', fontSize: '12px', lineHeight: '1.8' }}>
                  <span style={{ color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Descripción:</span>
                ...
                {observaciones && (
                  <div style={{ marginBottom: '16px' }}>
                    <span style={{ color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Observaciones:</span>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', marginBottom: '20px' }}>
                  <thead>
                    <tr style={{ borderBottom: '2px solid #a855f7' }}>
                      <th style={{ padding: '8px 10px', textAlign: 'left', color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Método de Pago</th>
                      <th style={{ padding: '8px 10px', textAlign: 'right', color: '#9333ea', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' }}>Monto</th>
                    </tr>
                  </thead>
                  <tbody>
          ...
          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '14px 16px', fontSize: '11px', lineHeight: '1.7', color: '#15803d' }}>
            ...
            <p style={{ margin: 0 }}>
              *El componente se verificó al momento de la entrega. La garantía cubre defectos de fábrica que no se hayan probado y se somete a una inspección previa para valer la garantía.
            </p>
            <p style={{ margin: '4px 0 0 0' }}>
              *El sello de garantía debe estar intacto. Golpes, daños por líquidos o manipulación no autorizada invalidan la garantía.
            </p>
          </div>
        ...
        <div className="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          ...
          <div className="flex min-h-screen items-center justify-center p-4 text-center sm:p-0 pointer-events-none">
            <div className="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md pointer-events-auto border border-slate-100 animate-in fade-in-0 zoom-in-95 duration-300 ease-out">
              <div className="h-1.5 w-full bg-gradient-to-r from-purple-500 via-violet-400 to-fuchsia-400" />
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-slate-500 mb-5 leading-relaxed">
                  Se generó correctamente el PDF para la nota de entrega de <strong className="text-slate-800">{component.nombre}</strong>. ¿Deseas actualizar el estado de disponibilidad del componente en el catálogo?
                </p>
                ...
                <div className="flex items-center gap-2 mb-6 px-4 py-3 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide ${component.disponibilidad === 'Disponible'
                    ? 'bg-green-100 text-green-700'
                    : component.disponibilidad === 'Coming soon'
                      ? 'bg-blue-100 text-blue-700'
                      : 'bg-red-100 text-red-700'
                    }`}>
                    {component.disponibilidad || 'No disponible'}
                  </span>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Útil si acabas de vender este componente y no hay más stock.
                      </p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Si esperas recibir más unidades de este componente pronto.
                      </p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Mantener el estado actual del componente sin realizar cambios.
                      </p>
                    </div>
                  </button>
                </div>
