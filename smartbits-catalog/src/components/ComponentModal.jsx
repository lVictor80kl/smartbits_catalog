...
export default function ComponentModal({ component, isOpen, onClose }) {
  ...
  const pVisual = component.estado?.pantalla ?? 10;
  const pFuncional = component.estado?.carcasa ?? 9;

  const TipoIcon = tipoIcons[component.tipo] || Package;

  const whatsappMessage = `Hola Smartbits, estoy interesado en el componente ${component.nombre} listado a $${component.precio}. ¿Pueden brindarme mayor información?`;
  ...
  const renderTypeSpecs = () => {
    switch (component.tipo) {
      case 'RAM':
        return (
          <>
            {component.generacion && <SpecItem icon={Zap} label="Generación" value={component.generacion} />}
            {component.velocidad && <SpecItem icon={Zap} label="Velocidad" value={component.velocidad} />}
            {component.capacidad && <SpecItem icon={Zap} label="Capacidad" value={component.capacidad} />}
          </>
        );
      case 'SSD':
        return (
          <>
            {component.capacidad && <SpecItem icon={HardDrive} label="Capacidad" value={component.capacidad} />}
            {component.interfaz && <SpecItem icon={HardDrive} label="Interfaz" value={component.interfaz} />}
            {component.tipo_ssd && <SpecItem icon={HardDrive} label="Tipo" value={component.tipo_ssd} />}
          </>
        );
      case 'Bateria':
        return (
          <>
            {component.capacidad_bateria && <SpecItem icon={BatteryFull} label="Capacidad" value={component.capacidad_bateria} />}
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-0 sm:p-4 md:p-6 lg:p-8" role="dialog" aria-modal="true">
      <div
            ...
            <div className="p-8 sm:p-12 lg:p-14 flex flex-col bg-white dark:bg-brand-950 transition-colors">
              <div className="mb-10">
                <div className="flex items-center gap-3 mb-4">
                  <TipoIcon className="w-5 h-5 text-brand-400 dark:text-brand-500" />
                  <p className="text-[10px] lg:text-xs font-black text-brand-400 dark:text-brand-500 uppercase tracking-[0.3em]">{component.tipo}</p>
                </div>
                <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black text-brand-800 dark:text-white leading-[1.05] mb-8 tracking-tighter">{component.nombre}</h3>

                <div className="flex flex-wrap items-center gap-8">
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl sm:text-5xl lg:text-6xl font-black text-brand-900 dark:text-white tracking-tighter">${component.precio}</span>
              ...
              <div className="bg-brand-50 dark:bg-brand-900 border border-brand-100/60 dark:border-brand-800/80 rounded-[3rem] p-8 lg:p-12 mb-12 relative overflow-hidden">
                ...
                <div className="space-y-10">
                  {[
                    { label: "Estado Visual", val: pVisual },
                    { label: "Estado Funcional", val: pFuncional }
                  ].map((st, i) => (
                    <div key={i}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-12 mb-14">
                {component.marca && <SpecItem icon={Package} label="Marca" value={component.marca} />}
                {renderTypeSpecs()}
              </div>

              {component.otros && component.otros.trim() !== "" && (
                <div className="others">
                  ...
                  <div className='flex items-center gap-4'>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-12 mb-14">
                      <SpecItem icon={Package} label="Otras Características" value={component.otros} />
                    </div>
                  </div>
                </div>
              )}
